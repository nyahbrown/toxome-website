...
// Single source of truth for "which products belong to this article". Used by
// the end-of-article Shop the edit rail (components/ShopTheEdit) AND by the
// Pinterest pin engine, which pins the article's photos one at a time.
...
//     men's/kids/home.
export async function selectEditProducts(article: Article): Promise<Product[]> {
...
// Every photo this article can be pinned with, in pin order: the editorial hero
// first, then each Shop-the-edit product shot. Up to 6 distinct pins per
// article, all linking back to the same piece — the multi-pin drip The Good
// Trade runs. Index into this array is the pin's identity (see
// /journal/[slug]/pin/[i] and the journal_pins ledger's photo_index).
export async function articlePhotoPool(article: Article): Promise<string[]> {
  const products = await selectEditProducts(article);
  const seen = new Set<string>();
  const out: string[] = [];
  for (const url of [article.hero, ...products.map((p) => p.item_image)]) {
