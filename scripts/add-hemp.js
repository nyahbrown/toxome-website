 ...
 * hemp sourcing pass; requires hemp in the parsed composition and inserts
 * as a draft for /admin review.
 *
 *   node --env-file=.env.local scripts/add-hemp.js <url> [--brand "Marine Layer"] [--gender Women] [--live]
 *   node --env-file=.env.local scripts/add-hemp.js --file urls.txt   # one url (+optional "|Brand") per line
 */
...
const HEMP_OK = (comp) => comp && Object.keys(comp).some((k) => /hemp/.test(k));
...
async function addOne(supabase, url, argBrand, gender, live) {
  ...
  if (!HEMP_OK(comp)) return { url, skip: `no hemp in comp ${JSON.stringify(comp)}` };
